...
Adds complete Elasticsearch index, search, mapping, aggregation, and bulk workflows as an installable connector extension.

This connector is listed in the public Irodori extension marketplace.

## Connector

- Extension ID: `irodori.elasticsearch`
- Engine ID: `elasticsearch`
- Wire: `search`
- Default port: `9200`
...
The Rust code keeps native ABI exports in `src/lib.rs`, shared buffer/JSON helpers in `src/abi.rs`, and Elasticsearch behavior in `src/driver.rs`.
...
- Inspired by: `Kibana Discover`, `Elasticsearch aggregations`, `Elasticsearch hybrid search`, `Elasticsearch kNN search`
...
| `query` | Runs SQL through the Elasticsearch SQL API. |
